...
export async function createUser(p: any) {
...
export async function updateUser(id: number, p: any) {
  await pool.query(
    `UPDATE usuarios
     SET
      nombre = ?,
...
export async function setUserVacationDays(id: number, dias: number) {
  await pool.query(
    `UPDATE usuarios
     SET dias_vacaciones_disponibles = ?, updated_at = NOW()
     WHERE id = ?`,
    [dias, id]
  );
}
